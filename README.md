...
HandlerDirSearch 是一个基于 Wails + Vue 开发的桌面应用程序，用于批量扫描和分析 URL。该工具可以从包含 HTTP 200 状态码的日志文件中提取 URL，并对这些 URL 进行并发扫描，提取页面标题、组件信息等关键数据，最终生成详细的 Markdown 格式报告。

## ✨ 功能特性

- 📁 **智能文件解析**：自动从日志文件中提取 HTTP 200 状态码对应的 URL
...
输入文件应包含 HTTP 状态码日志，程序会自动提取状态码为 200 的行中的 URL。

示例输入文件内容：
```
200 GET https://example.com/page1
404 POST https://example.com/page2
...
#### 新增功能
- ✨ 初始版本发布
- 📁 支持从日志文件中提取 HTTP 200 状态码对应的 URL
- 🚀 实现高效的并发扫描功能（支持自定义并发数）
- ⏱️ 支持灵活的请求超时控制
- 🔄 支持重定向跟随/不跟随选项
- 📊 自动生成 Markdown 格式的扫描报告
- 🎯 自动识别网页技术栈和组件
